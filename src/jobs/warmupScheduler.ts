...
async function warmupScheduler() {
  ...
  const sendWindowStart = await getWarmupConfig('send_window_start', env.SEND_WINDOW_START);
  const sendWindowEnd = await getWarmupConfig('send_window_end', env.SEND_WINDOW_END);
  ...
  const globalDailyLimit = await getWarmupConfig('global_daily_limit', env.DEFAULT_DAILY_LIMIT);
  ...
  for (const fromDomain of activeDomains) {
    ...
    if (sentTodayCount >= dailyLimit) {
      logger.info(`Domain ${fromDomain.fromEmail} skipped: daily limit reached (${dailyLimit}).`);
      continue;
    }

    // Calculate send probability with jitter
    const remainingToSend = dailyLimit - sentTodayCount;
    const remainingWindows = calculateRemainingWindows(sendWindowStart, sendWindowEnd, env.SCHEDULER_INTERVAL_MINUTES);
    const sendProbability = remainingWindows > 0 ? remainingToSend / remainingWindows : 0;

    if (Math.random() > sendProbability) {
      logger.info(`Domain ${fromDomain.fromEmail} skipped: send probability not met.`);
      continue;
    }
    ...
    if (otherDomains.length === 0) {
      logger.warn(`Domain ${fromDomain.fromEmail} has no other active domains to send to.`);
      continue;
    }
    ...
    if (success) {
      logger.info(`Warmup email sent from ${fromDomain.fromEmail} to ${toDomain.fromEmail}`);
    } else {
      logger.error(`Failed to send warmup email from ${fromDomain.fromEmail} to ${toDomain.fromEmail}`);
    }
  }
}

function calculateRemainingWindows(sendWindowStart: string, sendWindowEnd: string, intervalMinutes: number): number {
  ...
  const totalIntervals = Math.floor(totalWindowDurationMs / (intervalMinutes * 60 * 1000));
  const elapsedIntervals = Math.floor(elapsedWindowDurationMs / (intervalMinutes * 60 * 1000));
...
export function startWarmupScheduler() {
  ...
  cron.schedule(`*/${env.SCHEDULER_INTERVAL_MINUTES} * * * *`, warmupScheduler, {
    scheduled: true,
    timezone: "Etc/UTC" // Use a consistent timezone
  });
  logger.info(`Warmup scheduler started, running every ${env.SCHEDULER_INTERVAL_MINUTES} minutes.`);
